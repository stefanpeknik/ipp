...
def err_exit(exit_code: int, e: str):
    print(e, file=sys.stderr)
    sys.exit(exit_code)
...
class Interpret:
    def run(self):
        ...
        try:
            ...
            if args.source_file is None:
                ...
                try:
                    ...
                    err_exit(err.ERR_FILE_OPEN,
                             "Source file {0} not found.".format(args.source_file))
            if args.input_file is None:
                ...
                try:
                    ...
                    err_exit(err.ERR_FILE_OPEN,
                             "Input file {0} not found.".format(args.input_file))
        ...
        # catch exceptions
            ...
            err_exit(err.ERR_MISSING_PARAM, e)
        except InvalidXMLFormatException as e:
            err_exit(err.ERR_INVALID_XML_FORMAT, e)
        except InvalidXMLStructureException as e:
            err_exit(err.ERR_INVALID_XML_STRUCTURE, e)
        except SemanticException as e:
            err_exit(err.ERR_SEMANTIC_ERROR, e)
        except OperandTypeException as e:
            err_exit(err.ERR_OPERAND_TYPE_ERROR, e)
        except UndefinedVariableException as e:
            err_exit(err.ERR_UNDEFINED_VARIABLE, e)
        except FrameNotFoundException as e:
            err_exit(err.ERR_FRAME_NOT_FOUND, e)
        except MissingValueException as e:
            err_exit(err.ERR_MISSING_VALUE, e)
        except InvalidOperandValueException as e:
            err_exit(err.ERR_INVALID_OPERAND_VALUE, e)
        except StringErrorException as e:
            err_exit(err.ERR_STRING_ERROR, e)
        except InternalErrorException as e:
            err_exit(err.ERR_INTERNAL, e)
        finally:
